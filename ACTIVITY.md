...
- `docs/superpowers/plans/2026-09-17-phase-2-shared-types-platform-settings.md` (Phase 2: shared domain types, `platform_settings`, `audit` foundation) is **fully implemented**. `audit`/`platform_settings` are foundation apps (shared infrastructure other domain apps build on), not domain apps themselves — `accounts`, `listings`, etc. (spec Phase 2+'s actual domain apps) are still not started.

...
def test_render_email_substitutes_variables_into_subject_and_body():
    ...
    subject, html = render_email("password_reset", "EN", {"name": "Alex", "url": "https://nautelo.com/reset"})
...
def test_render_email_falls_back_to_english_when_the_locale_is_missing():
    EmailTemplate.objects.create(key="password_reset", locale="EN", subject="EN subject", html_body="<p>EN body</p>")

    subject, html = render_email("password_reset", "IT", {})
...
def test_render_email_prefers_the_exact_locale_over_the_english_fallback():
    EmailTemplate.objects.create(key="password_reset", locale="EN", subject="EN subject", html_body="<p>EN</p>")
    EmailTemplate.objects.create(key="password_reset", locale="IT", subject="IT subject", html_body="<p>IT</p>")
...
def test_render_email_wraps_the_body_in_the_shared_layout():
    EmailTemplate.objects.create(key="password_reset", locale="EN", subject="s", html_body="<p>unique-marker</p>")

    _, html = render_email("password_reset", "EN", {})
...
def test_render_email_escapes_html_in_a_variable_value():
    EmailTemplate.objects.create(key="password_reset", locale="EN", subject="s", html_body="<p>{{ name }}</p>")

    _, html = render_email("password_reset", "EN", {"name": "<script>alert(1)</script>"})

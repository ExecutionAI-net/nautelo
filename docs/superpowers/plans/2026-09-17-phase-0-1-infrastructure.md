  ...
  backend:
    ...
    steps:
      ...
      - name: Run tests
        ...
        working-directory: backend
        run: |
          if find . -type f -name "test_*.py" -not -path "./.venv/*" | grep -q .; then
            uv run pytest -v
          else
...
DATABASE_URL=postgres://nautelo:nautelo@127.0.0.1:5433/nautelo
REDIS_URL=redis://127.0.0.1:6380/0
CELERY_BROKER_URL=redis://127.0.0.1:6380/1
CELERY_RESULT_BACKEND=redis://127.0.0.1:6380/2
CHANNELS_REDIS_URL=redis://127.0.0.1:6380/3
...
OBJECT_STORAGE_ENDPOINT_URL=http://127.0.0.1:9010
OBJECT_STORAGE_ACCESS_KEY=nautelo
OBJECT_STORAGE_SECRET_KEY=nautelo123
OBJECT_STORAGE_BUCKET_NAME=nautelo-media
OBJECT_STORAGE_REGION=us-east-1
...
- **`config/wsgi.py`'s `DJANGO_SETTINGS_MODULE` fallback is structurally unable to default to anything other than what `config/celery.py` chooses, and is therefore NOT fail-secure.** Root cause: `config/__init__.py` does `from .celery import app as celery_app` (the standard Celery+Django bootstrap pattern, needed for task autodiscovery) — since `wsgi.py`/`asgi.py` are submodules of the `config` package, Python always runs `config/__init__.py` in full (including celery.py's own `os.environ.setdefault(..., "config.settings.dev")`) before either file's own top-level code executes, so any attempt to set a different default in `wsgi.py` is silently a no-op (`setdefault` never overwrites an already-set value). Verified empirically 2026-09-17: importing `config.wsgi` with `DJANGO_SETTINGS_MODULE` unset in the environment resolves `DEBUG=True` (dev settings) even after changing wsgi.py's own line to request `config.settings.prod`. This means a production WSGI server (gunicorn/uwsgi) that forgets to set `DJANGO_SETTINGS_MODULE` explicitly would silently boot with dev/DEBUG settings — a fail-open default in the one entry point whose only realistic real-world caller is a production server. `manage.py`'s own default is unaffected (it's an external script, not a `config` submodule, so its `setdefault` genuinely runs first). **Deferred to Phase 22** (deployment/hardening) rather than fixed now: not exploitable today since there is no live deployment yet, and the real fix requires deliberately restructuring the Celery/Django eager-import bootstrap, which carries real risk of subtly breaking task autodiscovery if done as a rushed side-fix. Phase 22 should either (a) require `DJANGO_SETTINGS_MODULE` to be set explicitly with no fallback in production deploy tooling (documented requirement, no code fallback needed), or (b) restructure `config/__init__.py`'s celery import to not force an env var default at all, letting Django's own `ImproperlyConfigured` error fail loudly if nothing set it — either resolves the fail-open gap without the current no-op illusion of a fix.

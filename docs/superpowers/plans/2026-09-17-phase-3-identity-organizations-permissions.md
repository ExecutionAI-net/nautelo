...
**Note (no `AuditEvent` write here):** spec §1 requires the auto-approval switch to be "changed by staff and audited". Phase 3 satisfies the *actor* half of that with the spec's own `auto_approve_changed_by` / `auto_approve_changed_at` columns and enforces staff-admin-only mutation. It deliberately writes no `AuditEvent` row here: the staff-facing endpoint `PATCH /api/v1/staff/brokers/<id>/approval-policy/` (spec §30.1) is Phase 12's deliverable, and auditing this action belongs there, not in this phase's service function, since Phase 3 exposes no API path that reaches `set_broker_auto_approval` (only the Django admin surface, gated to staff admins). Phase 12 must emit the `AuditEvent` both at its endpoint and inside `set_broker_auto_approval` itself once that endpoint exists. (Note: the `audit` app, Phase 2, is fully merged and stable as of this writing — the boundary here is one of scope ownership, not of a missing dependency.) This is a scope boundary, not a TODO placeholder: nothing in Phase 3 exposes an unaudited mutation path to a non-admin.
...
- **No staff impersonation.** Spec §12 item 6 says "Ensure staff impersonation, **if available**, is read-only by default and prominently audited." It is deliberately not available: it is simply out of scope for this phase, and a half-built impersonation switch would be exactly the partial implementation §39 forbids. (The Phase 2 `audit` app it would depend on is fully merged and stable as of this writing, so nothing here is blocked on a missing dependency — this is a scope decision, not a readiness gap.) If a later phase adds it, it must be read-only by default and must write an `AuditEvent` on every session.

...
**Ruling (controller-approved): accept as documented, no lock added — do not drop `of=("self",)`.** This is a judgment about rule 2's *intent*, not a quotation of it — rule 2 does not itself say "except under concurrency". `IsOwnerOrBrokerEditor`'s `broker__status=ACTIVE` filter already returns 403 for any request starting after a suspension commits, narrowing the real window to the microseconds between that permission check and this service's own unlocked read, not the whole request lifetime. Locking `BrokerOrganization` to close a window this narrow would serialize an entire agency's concurrent submissions for a benefit that small. Recorded in Known Limitation 10.
...
Each item names the phase that closes it. None breaks a MUST requirement *of this phase* (spec §39: "Any known limitation that breaks a MUST requirement prevents completion") — every one is either assigned elsewhere by the spec or an explicitly ruled scope boundary. Item 10's suspension race rests on a reading of §21 rule 2's intent rather than on its literal wording; the controller reviewed it and ruled to accept it as documented (see item 10) rather than lock the organization row.
...
10. **The whole `BrokerOrganization` row — both `auto_approve_listings` *and* `status`/`is_active` — is read without a lock at submit time.** Ruled in Scope rulings. Two distinct races follow from the one unlocked read:
    ...
    - **Suspension (`status` → `SUSPENDED`, i.e. `is_active` → False).** A submission whose permission check and locked listing fetch both ran before a staff admin's suspension committed can still auto-publish once afterwards. §21 rule 2's suspension clause is unconditional in its wording, so — unlike the toggle race — this one is *not* self-evidently covered by rules 5/6. **Ruling (controller-approved): accept as documented, no lock added.** Suspension is enforced twice on independent reads, the first being `IsOwnerOrBrokerEditor`'s `broker__status=ACTIVE` filter, which returns 403 for every request that starts after the suspension commits — narrowing the real race window to the microseconds between that permission check and the service's own read, not the whole request lifetime naively implied by "before a suspension committed." The remedy — `listings.decisions.suspend_listing`, spec §36.4 — is already merged and immediate, and suspension does not retroactively unpublish the organization's other live listings either, so a listing surviving this microsecond race is consistent with how suspension already behaves elsewhere in this codebase. Locking the organization row to close it entirely would serialize an agency's concurrent submissions behind that one row for a benefit this narrow — not worth it. Do not lock the broker row in Task 2 Step 3b.

...
| 5 | Directory consolidation | 3, 4 | **implementing** | `2026-09-18-phase-5-directory-consolidation.md` (merged, PR #34) | 17 tasks, ~5780 lines. Tasks 1-10 merged (PR #69-74, #76-78, #81); Task 10 (the spec §14.3 `import_legacy_directory` management command) needed one fix round for a real idempotency bug (a staff title rename after import could duplicate a `ProfessionalService` row on reimport), independently re-verified via mutation testing before merge. Tasks 11-17 (frontend directory/detail/SEO pages, redirects, whole-phase acceptance pass) remaining. See ledger for full detail. |

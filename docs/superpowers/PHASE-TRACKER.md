...
| 3 | Identity, organizations and permissions | 2 | **implementing** | `2026-09-17-phase-3-identity-organizations-permissions.md` | Went through 4 full rounds of review/fix given this phase's stakes (see plan/ledger for detail). **Task 1 merged** (PR #29 — custom `accounts.User` + `AUTH_USER_MODEL` migration, real local dev Postgres reset proven via command output, independent review PASS/APPROVED WITH MINOR NOTES, zero Critical/Important findings). Tasks 2-12 remain: request-ID middleware + error envelope; registration+email verification; JWT login/refresh/logout; `brokers`/`professionals` apps; staff groups+authorization services+DRF permission classes (privilege-escalation fixes already baked into the plan); `/api/v1/session/`; account+broker-team management API; frontend session/login/route-guards; final acceptance tests+ACTIVITY.md. |
...
| 11 | Listing workflow and immutable snapshots | 3, 4 | **ready** | `2026-09-18-phase-11-listing-workflow.md` (merged, PR #33) | 15 tasks, ~7780 lines. Went through review → fix → adversarial re-review: initial review found 3 Critical (staff corrections never reaching the published snapshot; wrong exception type in a test; a locking-contract contradiction between two tasks) + 9 Important; one fix pass closed them all; re-review independently traced the actual code paths for all 3 Criticals + 2 Importants and confirmed genuine fixes at the code level. **READY TO IMPLEMENT** — dispatch Task 1 once Phase 3 has bandwidth (never run its implementer in parallel with another Phase 3 task per the sequential-per-plan rule; Phase 11 is a separate plan so CAN run in parallel with Phase 3's current task). |
